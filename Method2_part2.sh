...
#====== TRB only BEGIN =========
cat $5/out/tophat_run/tophat_run/accepted_hits.bam | grep -v ^@ | awk '{print "@"$1"\n"$10"\n+\n"$11}' > $5/out/tophat_run/overlapping_reads.fq
cat $5/out/tophat_run/overlapping_reads.fq | awk 'NR%4==1{printf ">%s\n", substr($0,2)}NR%4==2{print}' > $5/overlapping_reads.fa

grep "/1" $5/overlapping_reads.fa > $5/overlapping_readsID.fa
sed 's\/1\\g' $5/overlapping_readsID.fa > $5/overlapping_readsID2.txt
sed 's\>\\g' $5/overlapping_readsID2.txt > $5/overlapping_readsID3.txt
...
#====== TRA only BEGIN =========
cat $5/out/tophat_run/accepted_hits.bam | grep -v ^@ | awk '{print "@"$1"\n"$10"\n+\n"$11}' > $5/out/tophat_run/overlapping_reads.fq
cat $5/out/tophat_run/overlapping_reads.fq | awk 'NR%4==1{printf ">%s\n", substr($0,2)}NR%4==2{print}' > $5/overlapping_reads.fa
